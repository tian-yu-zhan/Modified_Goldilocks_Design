...
# For simulation plot with multiple GD cutoffs
#add ppn^prime
setwd("~/goldilocks/")
...
power.cal = function(input){
  alpha = input$alpha
  alpha.combin = input$alpha.combin
  prior.a = input$prior.a
  prior.b = input$prior.b
  ...
  for (itt in 1:n.itt){
    ...
    if (pmax.fisher<p.max.cut){
      ...
      if ((p1<a0.fisher)&(p1*p2< c.fisher)) sign.mat[itt, 1] = 1
    ...
    if (pmax.inverse<p.max.cut){
      ...
      if ((p1<a0.inverse)&((1-pnorm(w1.inv*qnorm(1-p1)+w2.inv*qnorm(1-p2))) <(c.inverse*0.99))) sign.mat[itt, 2] = 1
      #if ((p1<a0.inverse)&((1-pnorm(w1.inv*qnorm(1-p1)+w2.inv*qnorm(1-p2))) <(c.inverse))) sign.mat[itt, 2] = 1
    }
...
for (scen.ind in 1:(2*19*5)){
#for (scen.ind in 1:(19)){
#for (scen.ind in c(1:5, 96:100)){
  #random.seed = NA
  random.seed = scen.ind
  ...
  alpha.combin.in = 0.025
  
  #scen.ind=1
  nc.1.un.in = (scen.ind%%19)*5
  if (scen.ind%%19==0) nc.1.un.in=95
  #print(nc.1.un.in)
  
  c.com.ind = (scen.ind-1)%/%19
  if ((c.com.ind==0)|(c.com.ind==5)) c.com.in = 0.025
  if ((c.com.ind==1)|(c.com.ind==6)) c.com.in = 0.022
  if ((c.com.ind==2)|(c.com.ind==7)) c.com.in = 0.02
  if ((c.com.ind==3)|(c.com.ind==8)) c.com.in = 0.015
  if ((c.com.ind==4)|(c.com.ind==9)) c.com.in = 0.01
  #print(c.com.in)
  ...
  rownames(output.table)[scen.ind] =
    paste("rate", rate.in, "delta", delta, "Sn", pn.cut.in, "Fn", p.max.cut.in,
          "n1", nc.1.in, "n1.un", nc.1.un.in, "c.com", c.com.in)
}
...
print(output.table)
write.csv(output.table, paste0("results/subset/same_type_1_error_multiple_plot_prime", ".csv"))
