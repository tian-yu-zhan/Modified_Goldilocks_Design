...
power.cal = function(input){
  ...
  fisher.unif = mean((p1.unif<a0)&(p1.unif*p2.unif<(c.fisher*0.99)))
  inverse.unif = mean((p1.unif<a0)&((1-pnorm(w1.inv*qnorm(1-p1.unif)+w2.inv*qnorm(1-p2.unif))) <(c.inverse*0.99)))
...
for (scen.ind in 1:24){
  ...
  output.table = rbind(output.table,
                       c(apply(power.fit$sign.mat, 2, mean), power.fit$dec.unif, mean(power.fit$dec.mat==1),
...
print(output.table)
write.csv(output.table, paste0("results/subset/same_type_1_error_com", ".csv"))
...
colnames(output.table) = c("fisher", "inverse", "goldilocks", "stop for futility",
                           "stop recruiting", "continue recruiting", "a0","min p1 coh 1",
                           "max p1 coh 2", "random seed", "n.itt")

print(Sys.time() - time.temp)
write.csv(output.table, "results/subset/same_output_power_com.csv")
